...
var $, ASTCoffeeScriptReplacer, ASTDepManager, ASTNamespaceWrapper, AST_TRANSFORMERS, BASENAME_REXP, BUILD_DIR, BUILD_ENV, CHANGEEXT_REXP, COFFEE, COFFEEKUP, HTML, JSTF, MUSTACHE, Module, REDIS_DB_INDEX, Redis, STYLUS, TRANSFORM_COFFEESCRIPT_REPLACER, TRANSFORM_NAMESPACE_WRAPPER, basename, cake, change_ext, ck, coffee, escodegen, esprima, ff_coffee, ff_coffeekup, ff_copy, ff_html, ff_js, ff_merge, ff_mustache, ff_stylus, freeDBFS, fs_coffee, fs_coffeekup, fs_html, fs_js, fs_merge, fs_mustache, fs_stylus, hogan, initDBFS, is_dbfs, mkdirp, mkdirp_async, mkdirp_sync, read, remove_ext, resolve_module_dir, s_dbfs, sf_coffee, sf_coffeekup, sf_html, sf_js, sf_merge, sf_mustache, sf_stylus, ss_coffee, ss_coffeekup, ss_html, ss_js, ss_merge, ss_mustache, ss_stylus, stylus, write, _createRedisClient, _ref;
...
basename = function(path) {
  return BASENAME_REXP.exec(path)[1];
};

remove_ext = function(filename) {
  return filename.replace(CHANGEEXT_REXP, '');
};

change_ext = function(filename, ext) {
  if (ext[0] !== '.') {
    ext = '.' + ext;
  }
  return filename.replace(CHANGEEXT_REXP, ext);
};
...
resolve_module_dir = function(req) {
  var path;
  path = require.resolve(req);
  path = $.dirname(path);
  while (!($.existsSync(path + "/package.json") && path !== "/")) {
    path = $.dirname(path);
  }
  return path;
};
...
module.exports = {
  ...
  cake: cake,
  mkdirp: mkdirp,
  resolve_module_dir: resolve_module_dir,
  COFFEE: COFFEE,
  STYLUS: STYLUS,
  COFFEEKUP: COFFEEKUP,
  MUSTACHE: MUSTACHE,
